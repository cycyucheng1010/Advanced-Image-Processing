...
class MyWidget(QWidget):
	...
	def histogram_Equalization(self):
		...
		try:
			...
			# do Equalization
			self.gray_image = cv2.cvtColor(self.img,cv2.COLOR_BGR2GRAY)
			hist,bins = np.histogram(self.gray_image.flatten(),256,[0,256])
			cdf = hist.cumsum()
			cdf_normalized = cdf * hist.max() / cdf.max()
			img_equalized = np.interp(self.gray_image.flatten(),bins[:-1],cdf_normalized)
			img_equalized = img_equalized.reshape(self.gray_image.shape)
			img_equalized = np.uint8(img_equalized)
			cv2.imwrite('Equalization_gray.png', img_equalized)
			# show Equalization histogram
			self.saveEqualizationHistogram()
			self.gray_histogram = cv2.imread('gray_level_equalization_histogram.png')
			down_width = 300
			down_height = 200
			down_points = (down_width, down_height)
			self.gray_histogram = cv2.resize(self.gray_histogram, down_points, interpolation= cv2.INTER_LINEAR)
			...
			#show Equalization image
			self.canvas2 = cv2.imread('Equalization_gray.png')
			qimg = QImage(self.canvas2,self.width,self.height,self.bytesPerline,QImage.Format_RGB888)
			self.canvas2 = QPixmap(360,360).fromImage(qimg)
			self.label2.setPixmap(self.canvas2)
		except Exception as e:
	...
	def saveEqualizationHistogram(self):
		self.gray_image = cv2.cvtColor(cv2.imread('Equalization_gray.png'),cv2.COLOR_BGR2GRAY)
		plt.title('image')
		plt.xlabel('Intensity')
		plt.ylabel('Frequency')
		#hist,bin = np.histogram(self.gray_image.ravel(),256,[0,255])
		...
		plt.title('gray level histogram')
		plt.hist(self.gray_image.ravel(), 256, [0, 256])
		plt.savefig('gray_level_equalization_histogram.png')
		plt.close()
